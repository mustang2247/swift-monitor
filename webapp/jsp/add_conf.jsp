  ...
  <div class="tab_container">
			<table align="center" cellspacing="0" width=80%> 
				<tr>
				   ...
				   <td height=50><input type="text" name="starttime" maxlength=20></td>
				</tr> 
				<tr>
				   <td height=50>抓取间隔</td>
				   <td height=50><input type="text" name="interval" maxlength=20>秒</td>
				</tr> 
					...
					<tr width=500>
					   ...
					   <td height=50 colspan=13 >
					     <c:forEach var="proxy" items="${proxymap}">
					         <input name="proxy" type="checkbox"  value="${proxy.ID}"/>${proxy.IP}:${proxy.PORT}
					   	 </c:forEach>
					   </td>
					</tr> 
				</c:if>
					...
					<tr width=500>
					   ...
					   <td height=50 colspan=13 >
					     <c:forEach var="seed" items="${inseedmap}">
					         <input name="inseed" type="checkbox" value="${seed.ID}" />${seed.INSIDE_NAME}
					   	 </c:forEach>
					   </td>
					</tr> 
				</c:if>
				<c:if test="${fn:length(outseedmap) > 0}">
					<tr width=500>
					   ...
					   <td height=50 colspan=13 >
					     <c:forEach var="seed" items="${outseedmap}">
					         <input name="outseed" type="checkbox" value="${seed.ID}" />${seed.INSIDE_NAME}
					   	 </c:forEach>
					   </td>
					</tr> 
				</c:if>
				   ...
				   <td height=50><input type="text" name="indelay" maxlength=10></td>
				   <td height=50>外置种子抓取延迟</td>
				   <td height=50><input type="text" name="outdelay" maxlength=10></td>
				</tr>
				<tr>
				   <td height=50>内置种子是否支持下载</td>
				   <td height=50><input type="radio" name="inisdownload" value="1">是 <input type="radio" name="inisdownload" value="0">否</td>
				   <td height=50>外置种子是否支持下载</td>
				   <td height=50><input type="radio" name="outisdownload" value="1">是 <input type="radio" name="outisdownload" value="0">否</td>
				</tr>
				<tr>
				   <td height=50>内置种子是否支持JS</td>
				   <td height=50><input type="radio" name="jssupport" value="1">是 <input type="radio" name="jssupport" value="0">否</td>
				   <td height=50>外置种子是否支持级联</td>
				   <td height=50><input type="radio" name="iscascade" value="1">是 <input type="radio" name="iscascade" value="0">否</td>
				</tr>
				<tr>
				   <td height=50>外置种子抓取深度</td>
				   <td height=50><input type="text" name="depth" maxlength=10></td>
				</tr>
